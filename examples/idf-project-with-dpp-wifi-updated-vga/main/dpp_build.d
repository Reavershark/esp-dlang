...
void main()
{
  ...
  if (!fileList.empty)
  {
    auto dubRunPart = ["dub", "run", "--yes", "--build=release", "--compiler=dmd", "dpp@0.5.5", "--"];
    auto dppSingleArgsPart = ["--preprocess-only", "--no-sys-headers"];
    auto commandWithoutFiles = dubRunPart ~ dppSingleArgsPart ~ defineArgs ~ ignoreSymbolArgs ~ includeArgs;
    foreach(file; (fileList)) // parallel
    {
...
string[] includeArgs()
{
    ...
    foreach (componentIncludePath; [
        ...
        "freertos/config/include/freertos", "freertos/config/xtensa/include",
        "freertos/FreeRTOS-Kernel-SMP/portable/xtensa/include/freertos/"
    ])
    {
